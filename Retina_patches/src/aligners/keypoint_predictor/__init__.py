...
class KeypointPredictor(BaseAligner):
        ...
        if self.config.face_detector:
            ...
            if self.config.face_detector.startswith('dfa'):
                if self.config.face_detector == 'dfa_mobilenetv4':
                    pretrain_path=f'Retina_patches/src/aligners/keypoint_predictor/pretrained_models/aligners/dfa_mobilenetv4_medium/mobilenetv4_Final.pth'
                else:
    ...
    def forward(self, x):
        ...
        x = x.to(self.device) # shape 1,B,3,224,224
        x= x.squeeze(0)  # shape B,3,224,224
