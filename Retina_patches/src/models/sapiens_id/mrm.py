...
class MaskedRecognitionModel(nn.Module):
    ...
    def mrm_forward(self, images, foreground_masks=None, ldmks=None, subset_method=None):
        ...
        # preproc ldmks
        ldmks, body_bboxes, face_bboxes = self.kp_prprocessor(ldmks, self.training) # ldmks: B,19,2 body_bboxes: B,4 face_bboxes: B,4
        ...
        square_pos_embed=square_pos_embed.to(ldmks.device)
    ...
    def forward(self, x, foreground_masks=None, ldmks=None, return_intermediate=False, **kwargs):
        foreground_masks=foreground_masks.to(x.device)
        ldmks=ldmks.to(x.device)
        ...
        return feat[:,:,:] # experimental
        # feat: B,N(=253),512 ids_keep: B,251 ldmks_embs: B,512,19, pos_embed_masked: B,251,512 , used_ldmks: B,19,2
        ...
        if isinstance(out_head, tuple):
            final_feat, out_head = out_head # final_feat:B,4096 
        else:
            final_feat = out_head
