...
from models import D3_model_custom as D3_model
from models.D3_model_custom import TransformerClassifier, LocalTemporalClassifier, CosineTemporalClassifier
...
if __name__ == '__main__':
    ...
    parser.add_argument('--classifier-type', type=str, default='Transformer', choices=['Transformer', 'local_temporal','CosineTemporalClassifier'])
    ...
    if args.classifier_type == 'Transformer':
        ...
        classifier_model=TransformerClassifier(in_dim=768).to(device)
        
    elif args.classifier_type == 'local_temporal':
        classifier_model=LocalTemporalClassifier(num_patches=252).to(device)
        
    elif args.classifier_type == 'CosineTemporalClassifier':
        classifier_model=CosineTemporalClassifier(num_patches=125).to(device)
    ...
    optimizer = torch.optim.AdamW(list(classifier_model.parameters())+list(model.encoder.vision_model.embeddings.parameters()), lr=args.lr, weight_decay=1e-4)
    ...
    for epoch in range(args.epochs):
        ...
        for batch_frames, batch_label in tqdm(train_loader, desc=f"Epoch [{epoch+1}/{args.epochs}]"):
            ...
            outputs = model.forward_custom_patch(batch_inputs, ldmks=ldmks)
